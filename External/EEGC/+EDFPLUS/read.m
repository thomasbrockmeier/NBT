...
import EDFPLUS.signal_types;
import EDFPLUS.dimension_prefixes;
...
THIS_OPTIONS = {'starttime', 'endtime', 'startrec', 'endrec', ...
    ...
    'ignoreref', 'reflabel', 'signaltype', 'hdr'};
...
ignoreref = globals.evaluate.IgnoreRef;
reflabel = globals.evaluate.RefLabel;
hdr = [];
...
if isempty(hdr),
    if verbose,
        fprintf('\n(+EDFPLUS/read) Reading file header...');
    end
    if fid < 0,        
        error('EDFPLUS:read_edfplus:invalidFile', ...
    ...
    if verbose,
        if isfield(hdr.pid, 'code'),
            fprintf('\n(+EDFPLUS/read) :::: Patient code is ''%s''',...
                hdr.pid.code);
        end
        if isfield(hdr.pid, 'sex'),
            fprintf('\n(+EDFPLUS/read) :::: Patient sex is ''%s''',...
                hdr.pid.sex);
        end
        if isfield(hdr.pid, 'birthdate'),
            fprintf('\n(+EDFPLUS/read) :::: Patient birthdate is %s',...
                hdr.pid.birthdate);
        end
        if isfield(hdr.pid, 'name'),
            fprintf('\n(+EDFPLUS/read) :::: Patient name is ''%s''',...
    ...
    if ~strcmpi(tmp{1}, 'startdate'),
        if verbose,
            fprintf('\n');
            warning('EDFPLUS:read:invalidHeader',...
    ...
    if verbose,
        fprintf('\n(+EDFPLUS/read) :::: Recording started on date %s at time %s',...
    ...
    if ~ismember(lower(edfplus_type), {'edf+d','edf+c'}),
        ...
        if verbose,
            fprintf('\n(+EDFPLUS/read) :::: This is an %s file',upper(edfplus_type));
        end
        hdr.edfplus_type = edfplus_type;
    end
    ...
    if verbose,
        fprintf('\n(+EDFPLUS/read) :::: File contains %d data records of %4.3f seconds each', ...
            hdr.nrec, hdr.dur);
    end
    ...
    for i = 1:numel(hdr.label)
        ...
        if ~is_valid_signal_type && nargout > 1
            if verbose,
                fprintf('\n');
                warning('EDFPLUS:read:unknownSignalType', ...
                    'Unknown signal type ''%s'' in channel %d. This channel will be ignored.', ...
                    tmp{1}, i);
                fprintf('\n');
                hdr.channel_type{i} = 'Unknown';
            end
        elseif ignoreref && length(tmp)>1 && strcmp(tmp{2},reflabel),
            if verbose,
                fprintf('\n');
                warning('EDFPLUS:read:ignoringRefSignal', ...
                    'Channel %d with label ''%s'' is a reference lead. I will ignore it.', ...
                    i, strtrim(hdr.label{i}));
                fprintf('\n');
            end
            hdr.channel_type{i} = 'REF';
        elseif is_valid_signal_type && (isempty(signaltype) || ...
                ismember(tmp(1), signaltype)),
            signal_type_idx(i) = loc;
            hdr.is_signal(i) = true;
    ...
    if verbose,
        fprintf('\n(+EDFPLUS/read) :::: File contains %d signals and %d EDF annotation channels', ...
            length(find(hdr.is_signal)), length(find(hdr.is_annotation)));
    end
    ...
    for i = 1:numel(hdr.physdim)
        ...
        if isempty(pos),
            if verbose,
                fprintf('\n');
                warning('EDFPLUS:read:unknownDimension', ...
            ...
            if pos > 1,
                ...
                if is_valid_prefix,
                    ...
                    if verbose,
                        fprintf('\n');
                        warning('EDFPLUS:read:unknownDimensionPrefix', ...
    ...
    if ~isempty(wrong_chans),
        if verbose,
            fprintf('\n');
            warning('EDFPLUS:read:maxLessThanMin',...
    ...
    if status,
        fclose(fid);
        error('EDFPLUS:read:fseekFailed', 'Fseek failed');
    end
    ...
    if verbose,
        fprintf('\n(+EDFPLUS/read) Done reading file header\n');
    end
    ...
    if abs(hdr.record_size*hdr.nrec+hdr.header_size-hdr.file_size)>0,
        ...
        if header_duration ~= true_duration,
            ...
            if isinteger(true_nrec),
                if verbose,
                    warning('EDFPLUS:read:inconsistentHeader', ...
                        'Inconsistent header information. This has been corrected');
                    hdr.nrec = true_nrec;
                end
            else
                fclose(fid);
                error('EDFPLUS:read:inconsistentHeader', ...
...
if ~any(tf) || nargout < 3,
    if verbose && strcmpi(hdr.edfplus_type, 'edf+d'),
        fprintf('\n');
        warning('EDFPLUS:read:noncontiguousRecords',...
            'Assuming continuous records in an EDF+D file!');
        fprintf('\n');
        
    elseif verbose,
        fprintf('\n');
        warning('EDFPLUS:read:invalidFile',...
    ...
    if verbose,
        fprintf('\n(+EDFPLUS/read) Reading annotations...');
    end
    ...
    if verbose,
        fprintf('\n(+EDFPLUS/read) Done reading annotations\n');
    end
...
if nargout > 1 && ~onlyannotations,
    if verbose,
        fprintf('\n(+EDFPLUS/read) Reading signal values...');
    end
    if isempty(chan_idx),
        chan_idx = find(hdr.is_signal);
    end
    
    if isempty(chan_idx),
        if verbose,
            fprintf('\n');
            warning('EDFPLUS:read:noSignals', ...
    ...
    if (~isempty(endrec) && endrec > hdr.nrec) || ...
        ...
        fclose(fid);
        error('EDFPLUS:read:invalidRecordRange',...
    ...
    if starttime < rec_onset(1),
        fclose(fid);
        error('EDFPLUS:read:invalidTimeRange', ...
    ...
    if (starttime < rec_onset(startrec)) || (endtime > rec_onset(endrec)+hdr.dur),
        if verbose,
            fprintf('\n');
            warning('EDFPLUS:read:invalidTimeRange', ...
                'The requested time range does not exist.');
            fprintf('\n');
        end
        time = [];
        dat = [];
    ...
    if condition,
        ...
        while rec_count <= n_rec
            ...
            if read_count < block_size,
                fclose(fid);
                error('EDFPLUS:read:freadFailed', 'fread failed');
            end
            ch_data = ch_data';
    ...
    elseif ns < NS_THRESHOLD
        ...
        while rec_count <= n_rec
            ...
            for chan_itr = 1:ns
                ...
                dat(chan_itr, (rec_count-1)*spr+ipos) = ...
                    hdr.cal(chan_idx(chan_itr))*ch_data(this_offset(chan_itr)+(1:length(ipos))); 
    ...
    if verbose,
        fprintf('\n(+EDFPLUS/read) Done reading signal values\n');
    end
...
if ~isempty(tal_cell),
    if verbose,
        fprintf('\n(+EDFPLUS/read) Computing sample indices for the annotations...');
    end
    ...
    sr = max(hdr.sr(hdr.is_signal));
    spr = max(hdr.spr(hdr.is_signal));
    n_rec = endrec - startrec + 1;
    rec_onset_datarange = rec_onset(startrec:endrec);
    beg_sample = floor((starttime-rec_onset(startrec))*sr)+1;
    end_sample = min(ceil((endtime-rec_onset(endrec))*sr),spr);
    ...
    if verbose,
        fprintf('\n(+EDFPLUS/read) Done computing sample indices\n');
    end
...
if verbose,
    ...
    fprintf(['\n(+EDFPLUS/read) It took %' num2str(ndigits) '.0f seconds to read the file'], t);
    fprintf('\n');
end
