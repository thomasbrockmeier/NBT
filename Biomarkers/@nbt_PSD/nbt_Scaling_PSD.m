function [PSDobject] = nbt_Scaling_PSD(PSDobject,data,SignalInfo,PSD_plot,PSD_LowFrequencyFit,PSD_HighFrequencyFit)
%
...
PSDobject = nbt_UpdateBiomarkerInfo(PSDobject, SignalInfo);
PSDobject.PSD_LowFrequencyFit = PSD_LowFrequencyFit;
PSDobject.PSD_HighFrequencyFit = PSD_HighFrequencyFit;

%******************************************************************************************************************
for ChId = 1:size(data,2)
    signal = data(:,ChId);
[P,S]=psd(signal,length(signal),Fs,hanning(length(signal)));
...
PSDobject.PSDlogx = PSD_logx ;
PSDobject.PSDlogy{ChId,1} = PSD_logy;
PSDobject.MarkerValues(ChId,1) = PSD_exp;
PSDobject = nbt_UpdateBiomarkerInfo(PSDObject, SignalInfo);
end
