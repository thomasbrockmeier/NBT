...
function G = nbt_selectchansregs(G)

try
    path = G(1).fileslist(1).path;
    ...
    SignalInfo = load([path '/' name(1:end-13) '_info.mat'],Firstfield);
    ...
    Firstfield = Infofields{1};
    clear Loaded Infofields;
    SignalInfo = load([path name],Firstfield);
    SignalInfo = eval(strcat('SignalInfo.',Firstfield));
end
...
set(Chans_RegsSelection,'CreateFcn','movegui')
hgsave(Chans_RegsSelection,'onscreenfig')
close(Chans_RegsSelection)
Chans_RegsSelection = hgload('onscreenfig');
currentFolder = pwd;
...
chans = length(SignalInfo.Interface.EEG.chanlocs);
