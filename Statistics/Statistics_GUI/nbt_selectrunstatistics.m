        ...
        elseif length(group_ind) == 2
            ...
            if(NBTelementState)
                ...
                for l = 1:length(bioms_name)
                    ...
                    NBTelementCall = [NBTelementCall '];Condition, {''' Group1.selection.con{1,1}];
                    try
                        for m=2:size(Group1.selection.con,1)
                            NBTelementCall = [NBTelementCall ''',''' Group1.selection.con{m,1}];
                        end
                    catch
                    end
                    if(~isempty(FreqBand))
                    ...
                    if(isempty(tmp)) %ok this biomarker is empty
                ...
                for l = 1:length(bioms_name)
                    ...
                    NBTelementCall = [NBTelementCall '];Condition, {''' Group2.selection.con{1,1}];
                    try
                        for m=2:size(Group2.selection.con,1)
                            NBTelementCall = [NBTelementCall ''',''' Group2.selection.con{m,1}];
                        end
                    catch
                    end
                    if(~isempty(FreqBand))
                ...
                %% TOFIX Unit
                unit = cell(size(B_values1_cell,1), size(B_values1_cell,2));
                %
            else % not using NBTelements
