...
function [Signal, SignalInfo] = nbt_filterFIR(Signal, SignalInfo,hp,lp,fir_order,removeOffset)
narginchk(5, 6)
%
% Modified klaus.linkenkaer@cncr.vu.nl, 070521.
% Modified S. Poil, 12-Feb-2008, added warning for wrong filter order.
% Modified S.-S. Poil, 28-Aug-2014, from nbt_filter_fir to new format
%
%******************************************************************************************************************
% Purpose...
%
...
for ii = 1:size(Signal,2)
    Signal_filtered(:,ii) = filter(b,1,Signal(:,ii));
end
if(exist('removeOffset','var'))
    Signal_filtered = Signal_filtered((SignalInfo.convertedSamplingFrequency*fir_order*hp*2):end,:);
end
Signal = Signal_filtered;
SignalInfo.frequencyRange = [hp lp];
SignalInfo.filterSettings.filter  = 'nbt_filterFIR';
SignalInfo.filterSettings.fir_order = fir_order;
