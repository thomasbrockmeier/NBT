...
switch lower(g.icatype)
    case 'runica'
        ...
        if tmprank == size(tmpdata,1) | pca_opt
            % [EEG.icaweights,EEG.icasphere] = runica( tmpdata, 'lrate', 0.001, g.options{:} ); %temporal domain
            warning('Using spectral domain ICA - remember to set correct Fs ')
            [EEG.icaweights,EEG.icasphere] = runica( tmpdata, 'lrate', 0.001, 'specgram',[500 0.5 45 size(tmpdata,2) size(tmpdata,2)] );
        else
            ...
            warning('Using spectral domain ICA - remember to set correct Fs') % [Fs low high . .])
            [EEG.icaweights,EEG.icasphere] = runica( tmpdata, 'lrate', 0.001, 'pca', tmprank, 'specgram',[1000 0.5 45 size(tmpdata,2) size(tmpdata,2)] );
